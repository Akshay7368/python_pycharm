...
Bottoms = Frame(root, bg="orange", width = 1600, height=50)
Bottoms.pack(side = BOTTOM)

f1 = Frame(root, width = 1000, height=700)
f1.pack(side=LEFT)

f2 = Frame(root, width = 600,height=700)
f2.pack(side=RIGHT)
...
#-----------------INFO TOP------------
lblinfo = Label(Tops, font=('aria' ,30, 'bold'),text="Restaurant Management System", fg="sky blue", bd=10)
lblinfo.grid(row=0,column=0)
lblinfo = Label(Tops, font=('aria' ,20), text=currenttime,fg="steel blue")
lblinfo.grid(row=1,column=0)

lbinfo1 = Label(Bottoms, font=('aria', 20), text="Thankyou", fg="black", bd=10)
lbinfo1.grid(row=0,column=0)
lbinfo1 = Label(Bottoms, font=('aria', 20), text="Mobile: 9080147368", fg="black", bd=10)
lbinfo1.grid(row=0,column=1)
lbinfo1 = Label(Bottoms, font=('aria', 20), text="Gmail: abc@gmail.com ", fg="black", bd=10)
lbinfo1.grid(row=0,column=2)
...
txtreference = Entry(f1,font=('ariel' ,16,'bold'), textvariable=rand , bd=6,insertwidth=5,bg="powder blue" ,justify='left')
...
def ref():
    ...
    with open('Restaurant.csv', mode='a',newline='') as file:
        w = csv.writer(file)
        w.writerow([randomRef,cof,colfries,cob,cofi,cochee,codr,costoffries,costoflargefries,costofmeal,costofburger,costofcheeseburger,costofdrinks,costoffilet])
